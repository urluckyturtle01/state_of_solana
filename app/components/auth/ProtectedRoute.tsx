...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children }) => {
  ...
  const isInternalAuthRoute = () => {
    return pathname && typeof pathname === 'string' && 
           (pathname.startsWith('/sf-dashboards') || pathname.startsWith('/dflow'));
  };
  ...
  }, [pathname, isAuthenticated, isLoading, hasCheckedAuth, checkAuthForRoute, openLoginModal, isInternalAuth]);
    
  // Set hasCheckedAuth to true immediately for faster page load
  useEffect(() => {
    // No delay - set immediately when auth is checked
