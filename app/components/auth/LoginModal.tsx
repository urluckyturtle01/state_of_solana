  ...
  const handleInternalLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Set authenticated state directly with section-specific storage
        setIsAuthenticated(true, section);
        
        // Track successful internal login
        trackAuthEvent('login', `internal-password-${section}`);
