...
import { WalletBalanceDataPoint, fetchWalletBalanceData, balanceCohortOrder, balanceCohortColors } from "@/app/api/overview/user-activity/walletBalanceData";
...
const WalletBalancePieChart: React.FC<WalletBalancePieChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
}) => {
  // State for main chart
  const [data, setData] = useState<WalletBalanceDataPoint[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  
  // State for modal
  const [modalData, setModalData] = useState<WalletBalanceDataPoint[]>([]);
  ...
  const [tooltip, setTooltip] = useState({
    visible: false,
    data: null as WalletBalanceDataPoint | null,
    x: 0,
    y: 0
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const balanceData = await fetchWalletBalanceData();
      
      if (balanceData.length === 0) {
        setError('No data available for wallet balances.');
        setData([]);
      } else {
        setData(balanceData);
      }
  ...
  const calculatePercentage = (value: number, dataSet: WalletBalanceDataPoint[]) => {
    const total = dataSet.reduce((sum, item) => sum + item.Wallets, 0);
    return total > 0 ? ((value / total) * 100) : 0;
  };

  // Get color scale
  const getColorScale = (chartData: WalletBalanceDataPoint[]) => {
    return scaleOrdinal({
      domain: chartData.map(d => d.Balance_Cohort),
      range: chartData.map(d => balanceCohortColors[d.Balance_Cohort] || '#cccccc')
    });
  };

  // Handle tooltip
  const handleTooltip = (isVisible: boolean, dataPoint: WalletBalanceDataPoint | null, event: React.MouseEvent, isModal: boolean = false) => {
      ...
      <div className="relative h-full w-full" ref={isModal ? modalChartRef : chartRef}>
        {tooltip.visible && tooltip.data && (
          <ChartTooltip
            left={tooltip.x}
            top={tooltip.y}
            title={tooltip.data.Balance_Cohort}
            items={[
              {
                label: 'Wallets',
                value: formatNumber(tooltip.data.Wallets),
                color: balanceCohortColors[tooltip.data.Balance_Cohort]
              },
              {
                label: 'Percentage',
                  ...
                  >
                    {(pie) => {
                      return pie.arcs.map((arc, index) => {
                        ...
                        const fillColor = colorScale(arcData.Balance_Cohort) as string;
    ...
    return (
      <div className="flex flex-col gap-2 max-h-[60vh] overflow-y-auto overflow-x-hidden pr-1">
        {chartData.map((item, index) => (
          <div key={`legend-${item.Balance_Cohort}-${index}`} className="flex items-start">
            <div 
              className="w-2.5 h-2.5 mr-2 rounded-sm mt-0.5 flex-shrink-0"
              style={{ background: balanceCohortColors[item.Balance_Cohort] }}
            ></div>
            <div className="flex flex-col">
              <span className="text-[11px] text-gray-300 break-words">
                {item.Balance_Cohort} <span className="text-gray-400">({calculatePercentage(item.Wallets, chartData).toFixed(1)}%)</span>
              </span>
            </div>
          </div>
        ))}
  ...
  const downloadCSV = () => {
    ...
    // Create CSV content
    const headers = ["Balance Cohort", "Wallets", "Percentage"];
    const csvContent = [
      headers.join(","),
      ...chartData.map(item => {
    ...
    link.setAttribute('download', `wallet_balance_cohorts_${new Date().toISOString().split("T")[0]}.csv`);
    ...
    <div className="h-full w-full relative" ref={chartRef}>
      ...
      {isModalOpen && (
        <Modal 
          ...
          subtitle="Wallets grouped by SOL balance"
        >
              ...
              <div className="w-[25%] pl-3">
                <div className="flex flex-col gap-5 pt-1 h-full overflow-hidden">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-semibold text-gray-300">Balance Cohorts</h4>
                    <button 
                      onClick={downloadCSV}
