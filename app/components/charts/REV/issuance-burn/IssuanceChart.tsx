...
// Chart colors
export const issuanceColors = {
  axisLines: '#374151',
  tickLabels: '#6b7280',
  grossIssuance: '#60a5fa', // blue
  netIssuance: '#a78bfa', // purple
  grid: '#1f2937',
};
...
export const getIssuanceChartMetrics = (colors: { gross: string, net: string }) => {
  return [
    {
    ...
    {
      key: 'net_sol_issuance',
      displayName: 'Net Issuance',
      shape: 'circle',
      color: colors.net
    }
  ];
...
const getAvailableChartMetrics = (data: IssuanceDataPoint[], colors: { gross: string, net: string }) => {
  if (!data || data.length === 0) return [];
  
  return getIssuanceChartMetrics(colors);
};

const IssuanceChart: React.FC<IssuanceChartProps> = ({ 
  currency, 
  isModalOpen = false, 
  onModalClose = () => {} 
}) => {
        ...
        >
          <ParentSize debounceTime={10}>
            {({ width, height }) => {
              ...
              const allValues = displayData.flatMap(d => [d.gross_sol_issuance, d.net_sol_issuance]);
              ...
              if (minValue < 0) {
                yMin = minValue * 1.1; // Add 10% padding to minimum value
              }

              // Use a linear scale for the issuance amount with 0 included in domain
              const issuanceScale = scaleLinear<number>({
                domain: [yMin, yMax],
                range: [innerHeight, 0],
                nice: true,
              });
                  ...
                  <Group left={margin.left} top={margin.top}>
                    ...
                    <AxisLeft 
                      ...
                      tickFormat={(value) => {
                        ...
                        // Format without decimal places for whole millions
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        title="SOL Issuance" 
        subtitle="Tracking gross and net SOL issuance over time"
      >
