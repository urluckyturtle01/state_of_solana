...
// Props for the MintChart component
...
const MintChart: React.FC<MintChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  onUpdateLegends
}) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const processedData = processDataForStackedBar(chartData, 'Mint_Amount');
      ...
      if (onUpdateLegends) {
        const legendData = mints.map(mint => {
          // Sum the mint amounts for this mint
          const mintTotal = chartData
            .filter(d => d.mint === mint)
            .reduce((sum, item) => sum + item.Mint_Amount, 0);
      ...
      console.error('[MintChart] Error loading chart data:', err);
                ...
                <svg width={width} height={height}>
                  <Group left={margin.left} top={margin.top}>
                    ...
                    {displayData.map((d, i) => {
                      ...
                      // Generate bars for each mint, sorted by mint amount
                      availableMints
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        title="Stablecoin Mint Amount" 
        subtitle="Monthly mint amounts by stablecoin"
      >
          ...
          <div className="flex h-full">
            ...
            <div className="w-[10%] h-full pl-3">
              <div className="flex flex-col gap-1 text-[11px] text-gray-300">
                ...
                {availableMints
                  .sort((a, b) => {
                    // Calculate total mint amount for each stablecoin
                    const aTotal = data
                      .filter(d => d.mint === a)
                      .reduce((sum, d) => sum + d.Mint_Amount, 0);
                    const bTotal = data
                      .filter(d => d.mint === b)
                      .reduce((sum, d) => sum + d.Mint_Amount, 0);
