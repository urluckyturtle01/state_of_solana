...
import { MintBurnDataPoint, fetchMintBurnData, getStablecoinColor, formatDate, formatCurrency } from '../../../../api/stablecoins/mint-burn/mintBurnData';
...
// Props for the BurnChart component
...
const BurnChart: React.FC<BurnChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  onUpdateLegends
}) => {
  // Main chart data
  const [data, setData] = useState<MintBurnDataPoint[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const chartData = await fetchMintBurnData();
      ...
      const mints = [...new Set(chartData.map(d => d.mint))];
      setAvailableMints(mints);
      
      // Process data for the stacked bar chart
      const processedData = processDataForStackedBar(chartData, 'Burn_Amount');
      setProcessedData(processedData);
      setFilteredData(processedData);
      
      // Update legends
      if (onUpdateLegends) {
        const legendData = mints.map(mint => {
          // Sum the burn amounts for this mint
          const burnTotal = chartData
            .filter(d => d.mint === mint)
            .reduce((sum, item) => sum + item.Burn_Amount, 0);
      ...
      console.error('[BurnChart] Error loading chart data:', err);
  ...
  const processDataForStackedBar = (rawData: MintBurnDataPoint[], valueField: 'Mint_Amount' | 'Burn_Amount'): StackedBarData[] => {
    // Group by month
    const dataByMonth = rawData.reduce((acc, curr) => {
      if (!acc[curr.Month]) {
        acc[curr.Month] = {
          month: curr.Month,
          date: new Date(curr.Month),
          total: 0
        };
        availableMints.forEach(mint => {
          acc[curr.Month][mint] = 0;
        });
      }
      
      // Add to the appropriate mint field
      acc[curr.Month][curr.mint] = (acc[curr.Month][curr.mint] || 0) + curr[valueField];
      
      // Update the total
      acc[curr.Month].total += curr[valueField];
      ...
      <div className="flex flex-col h-full">
        ...
        {tooltip.visible && tooltip.data && (
          <ChartTooltip
            ...
            items={availableMints
              .filter(mint => tooltip.data[mint] > 0)
              .sort((a, b) => tooltip.data[b] - tooltip.data[a])
              .map(mint => ({
                color: getStablecoinColor(mint),
                label: mint,
                value: formatCurrency(tooltip.data[mint]),
                shape: 'square'
              }))}
            top={tooltip.top}
            left={tooltip.left}
                ...
                <svg width={width} height={height}>
                  <Group left={margin.left} top={margin.top}>
                    ...
                    {displayData.map((d, i) => {
                      ...
                      // Generate bars for each mint, sorted by burn amount
                      availableMints
        ...
        <div className="h-[15%] w-full mt-1">
          <BrushTimeScale
            ...
            getDate={(d) => d.date}
            getValue={(d) => d.total}
            lineColor="#f43f5e" // Red color for burn chart
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        title="Stablecoin Burn Amount" 
        subtitle="Monthly burn amounts by stablecoin"
      >
          ...
          <div className="flex h-full">
            ...
            <div className="w-[10%] h-full pl-3">
              <div className="flex flex-col gap-1 text-[11px] text-gray-300">
                <div className="mb-2 text-gray-400 font-medium">STABLECOINS</div>
                {availableMints
                  .sort((a, b) => {
                    // Calculate total burn amount for each stablecoin
                    const aTotal = data
                      .filter(d => d.mint === a)
                      .reduce((sum, d) => sum + d.Burn_Amount, 0);
                    const bTotal = data
                      .filter(d => d.mint === b)
                      .reduce((sum, d) => sum + d.Burn_Amount, 0);
                  ...
                  .map(mint => (
                    <div key={mint} className="flex items-center mb-1">
                      <div 
                        className="w-2.5 h-2.5 rounded-sm mr-1.5" 
                        style={{ backgroundColor: getStablecoinColor(mint) }}
                      />
                      <span>{mint}</span>
                    </div>
                  ))
