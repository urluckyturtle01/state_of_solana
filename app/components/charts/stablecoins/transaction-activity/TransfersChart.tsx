...
const TransfersChart: React.FC<TransfersChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  legendsChanged
}) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (activityData.length === 0) {
        setError('No data available for stablecoin transfers.');
        setRawData([]);
        setStackedData([]);
        setFilteredData([]);
        setAvailableMints([]);
      } else {
        // Extract unique mints and calculate total transfers for each
        const mintTotals: Record<string, number> = {};
        activityData.forEach(d => {
          if (!mintTotals[d.mint_name]) {
            mintTotals[d.mint_name] = 0;
          }
          mintTotals[d.mint_name] += d.transfers;
        });
        
        // Sort mints by total transfers (descending)
                ...
                <svg width={width} height={height}>
                  <Group left={margin.left} top={margin.top}>
                    ...
                    {displayData.map((d, i) => {
                      ...
                      // Generate bars for each mint, sorted by transfers
                      availableMints
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Monthly transfer count of stablecoins on Solana"
      >
              ...
              {loading ? (
                ...
                <div className="flex flex-col gap-1 overflow-y-auto max-h-[500px] pr-1">
                  {availableMints.map((mint) => {
                    // Calculate total transfers for this mint
                    const totalTransfers = rawData
                      .filter(d => d.mint_name === mint)
                      .reduce((sum, item) => sum + item.transfers, 0);
                    ...
                    return (
                      <LegendItem
                        ...
                        color={getStablecoinColor(mint)}
                        shape="square"
                        tooltipText={formatNumber(totalTransfers)}
                      />
                    );
                  })}
                </div>
