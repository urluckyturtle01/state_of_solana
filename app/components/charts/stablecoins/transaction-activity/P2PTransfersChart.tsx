...
interface P2PTransfersChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
...
const P2PTransfersChart: React.FC<P2PTransfersChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  legendsChanged
}) => {
  ...
  const prepareStackedData = useCallback((data: TransactionActivityDataPoint[]): StackedBarData[] => {
    const groupedByMonth = data.reduce((acc, curr) => {
      ...
      // Use mint_name as the property name with p2p_transfers value
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const activityData = await fetchTransactionActivityData();
      if (activityData.length === 0) {
        ...
        // Extract unique mints and calculate total P2P transfers for each
        const mintTotals: Record<string, number> = {};
        activityData.forEach(d => {
          if (!mintTotals[d.mint_name]) {
            mintTotals[d.mint_name] = 0;
          }
          mintTotals[d.mint_name] += d.p2p_transfers;
        });
        
        // Sort mints by total P2P transfers (descending)
        ...
        const prepared = prepareStackedData(activityData);
                ...
                <svg width={width} height={height}>
                  <Group left={margin.left} top={margin.top}>
                    ...
                    {displayData.map((d, i) => {
                      ...
                      // Generate bars for each mint, sorted by p2p_transfers
                      availableMints
            ...
            {({ width, height }) => {
              ...
              return (
                <BrushTimeScale
                  ...
                  getValue={(d: any) => d.p2p_transfers}
                  lineColor="#2775ca"
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Monthly peer-to-peer transfer count of stablecoins on Solana"
      >
              ...
              {loading ? (
                ...
                <div className="flex flex-col gap-1 overflow-y-auto max-h-[500px] pr-1">
                  {availableMints.map((mint) => {
                    // Calculate total P2P transfers for this mint
                    const totalP2PTransfers = rawData
                      .filter(d => d.mint_name === mint)
                      .reduce((sum, item) => sum + item.p2p_transfers, 0);
                    ...
                    return (
                      <LegendItem
                        ...
                        color={getStablecoinColor(mint)}
                        shape="square"
                        tooltipText={formatNumber(totalP2PTransfers)}
                      />
                    );
                  })}
                </div>
...
export default P2PTransfersChart; 