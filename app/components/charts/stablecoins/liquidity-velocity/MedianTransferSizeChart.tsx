...
interface MedianTransferSizeChartProps {
...
const MedianTransferSizeChart: React.FC<MedianTransferSizeChartProps> = ({ 
  stablecoin, 
  isModalOpen = false, 
  onModalClose = () => {} 
}) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('[MedianTransferSizeChart] Error loading chart data:', err);
  ...
  const fetchModalData = useCallback(async () => {
    ...
    try {
      ...
      console.error('[MedianTransferSizeChart] Error loading modal chart data:', err);
      let message = 'Failed to load data.';
      if (err instanceof Error) {
        message = err.message || message;
      }
    ...
    (
      ...
      if (closestPoint) {
        ...
        setTooltipData({
          ...
          medianTransferSize: typedClosestPoint.Median_P2P_Transfer_Size,
          color: getStablecoinColor(activeStablecoin)
        });
        setTooltipLeft(xScale(typedClosestPoint.date) + margin.left);
        setTooltipTop(yScale(typedClosestPoint.Median_P2P_Transfer_Size) + margin.top);
      } else {
        setTooltipData(null);
        setTooltipLeft(null);
        setTooltipTop(null);
        ...
        >
          <ParentSize>
            {({ width, height }) => {
              ...
              const valueMax = Math.max(...displayData.map(d => d.Median_P2P_Transfer_Size));
              ...
              // Use orange color for median line
                    ...
                    <Group left={margin.left} top={margin.top}>
                      ...
                      {displayData.length > 0 && (
                        <LinePath
                          ...
                          y={(d) => {
                            // Type casting to access properties safely
                            const typedD = d as unknown as { Median_P2P_Transfer_Size: number };
                            return yScale(typedD.Median_P2P_Transfer_Size);
                          }}
                          stroke={lineColor}
                          strokeWidth={2.5}
                          strokeOpacity={0.8}
                  ...
                  {tooltipData && tooltipLeft != null && tooltipTop != null && (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs font-medium">{formatNumber(tooltipData.medianTransferSize)}</div>
                      </div>
                    </Tooltip>
                  )}
                </>
            ...
            {({ width, height }) => {
              ...
              return (
                <BrushTimeScale
                  ...
                  getValue={(d: any) => d.Median_P2P_Transfer_Size}
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle={`${modalStablecoin} median peer-to-peer transfer size over time`}
      >
...
export default MedianTransferSizeChart; 