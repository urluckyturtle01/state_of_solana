...
const VelocityChart: React.FC<VelocityChartProps> = ({
  ...
  const prepareData = useCallback((data: TransactionActivityDataPoint[]): ProcessedData[] => {
    ...
    })).filter(item => item.velocity > 0 && !isNaN(item.velocity));
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        setError('No data available for stablecoin velocity.');
        setRawData([]);
        setProcessedData([]);
        setFilteredData([]);
        setAvailableMints([]);
      } else {
        // Extract unique mints and calculate average velocity for each
        const mintVelocities: Record<string, { total: number; count: number }> = {};
        data.forEach(d => {
          if (d.velocity > 0 && !isNaN(d.velocity)) {
            if (!mintVelocities[d.mint_name]) {
              mintVelocities[d.mint_name] = { total: 0, count: 0 };
            }
            mintVelocities[d.mint_name].total += d.velocity;
            mintVelocities[d.mint_name].count += 1;
          }
        });
        
        // Sort mints by average velocity (descending)
    ...
    (
      ...
      if (closestPoint) {
        ...
        const closestYDistance = sameDate.reduce(
          (min, p) => {
            const yDistance = Math.abs(yScale(p.velocity) + margin.top - y);
        ...
        >
          <ParentSize>
            {({ width, height }) => {
              ...
              const velocityMax = Math.max(...displayData.map(d => d.velocity));
                  ...
                  {tooltipData && tooltipLeft != null && tooltipTop != null && (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs font-medium">{formatNumber(tooltipData.velocity)}</div>
                      </div>
                    </Tooltip>
                  )}
                </>
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Monthly velocity indicators of stablecoins on Solana"
      >
              ...
              {loading ? (
                ...
                <div className="flex flex-col gap-1 overflow-y-auto max-h-[500px] pr-1">
                  {availableMints.map((mint) => {
                    // Calculate average velocity for this mint
                    const mintData = processedData.filter(d => d.mintName === mint);
                    const avgVelocity = mintData.length > 0
                      ? mintData.reduce((sum, item) => sum + item.velocity, 0) / mintData.length
                      : 0;
                    ...
                    return (
                      <LegendItem
                        ...
                        color={getStablecoinColor(mint)}
                        shape="square"
                        tooltipText={formatNumber(avgVelocity)}
                      />
                    );
                  })}
                </div>
