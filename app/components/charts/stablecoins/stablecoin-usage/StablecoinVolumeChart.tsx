...
import {
  fetchStablecoinVolumeData,
  StablecoinVolumeDataPoint,
  formatCurrency,
  getStablecoinColor
} from "@/app/api/stablecoins/stablecoin-usage/stablecoinVolumeData";
...
interface StablecoinVolumeChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
  legendsChanged?: (legends: {label: string, color: string, value?: number}[]) => void;
}

interface ExtendedVolumeDataPoint extends StablecoinVolumeDataPoint {
  date?: Date;
}
...
const StablecoinVolumeChart: React.FC<StablecoinVolumeChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  legendsChanged
}) => {
  const [rawData, setRawData] = useState<ExtendedVolumeDataPoint[]>([]);
  ...
  const prepareStackedData = useCallback((data: ExtendedVolumeDataPoint[]): StackedBarData[] => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const volumeData = await fetchStablecoinVolumeData();
      if (volumeData.length === 0) {
        setError('No data available for stablecoin volume.');
        setRawData([]);
        setStackedData([]);
        setFilteredData([]);
        setAvailableMints([]);
      } else {
        const dataWithDates = volumeData.map(d => ({
          ...d,
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Transaction volume of stablecoins on Solana over time"
      >
...
export default StablecoinVolumeChart; 