...
interface StablecoinHoldersChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
...
const StablecoinHoldersChart: React.FC<StablecoinHoldersChartProps> = ({
  isModalOpen = false,
  onModalClose = () => {},
  legendsChanged
}) => {
  ...
  const prepareStackedData = useCallback((data: ExtendedStablecoinDataPoint[]): StackedBarData[] => {
    const groupedByDate = data.reduce((acc, curr) => {
      ...
      // Use mint_name as the property name, storing holders count
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (usageData.length === 0) {
        ...
        // Extract unique mint names and calculate total holders for each
        ...
        dataWithDates.forEach(d => {
          ...
          mintNameTotals[d.mint_name] += d.holders;
        });
        
        // Sort mint names by total holders (descending)
                ...
                <svg width={width} height={height}>
                  <Group left={margin.left} top={margin.top}>
                    ...
                    {displayData.map((d, i) => {
                      ...
                      // Generate bars for each mint name, sorted by holders count
                      availableMintNames
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Number of holders for stablecoins on Solana over time"
      >
              ...
              {loading ? (
                ...
                <div className="flex flex-col gap-1 overflow-y-auto max-h-[500px] pr-1">
                  {availableMintNames.map((mintName) => {
                    // Calculate total holders for this mint name
                    const totalHolders = rawData
                      .filter(d => d.mint_name === mintName)
                      .reduce((sum, item) => {
                        // Just use the latest data point for each mint
                        return Math.max(sum, item.holders);
                      }, 0);
                    ...
                    return (
                      <LegendItem
                        ...
                        color={getStablecoinColor(mintName)}
                        shape="square"
                        tooltipText={formatNumber(totalHolders)}
                      />
                    );
                  })}
                </div>
...
export default StablecoinHoldersChart; 