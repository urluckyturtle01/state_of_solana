...
import { 
  fetchTradingAppRevenueData, 
  TradingAppRevenueDataPoint,
  formatCurrency,
  getTradingAppColor
} from "@/app/api/protocol-revenue/dex-ecosystem/tradingAppRevenueData";
...
interface TradingAppRevenueChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
...
interface ExtendedTradingAppRevenueDataPoint extends TradingAppRevenueDataPoint {
  date?: Date;
}
...
const TradingAppRevenueChart: React.FC<TradingAppRevenueChartProps> = ({ 
  ...
  const [rawData, setRawData] = useState<ExtendedTradingAppRevenueDataPoint[]>([]);
  ...
  const prepareStackedData = useCallback((data: ExtendedTradingAppRevenueDataPoint[]): StackedBarData[] => {
    // Group data by month
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      console.log(`Fetching Trading App revenue data...`);
      const revenueData = await fetchTradingAppRevenueData();
      
      if (revenueData.length === 0) {
        console.error('No data returned from API');
        setError('No data available for Memecoin Trading App revenue.');
        setRawData([]);
        setStackedData([]);
        setFilteredData([]);
                        ...
                        .forEach(platform => {
                          ...
                          stackedBars.push(
                            <Bar
                              ...
                              width={barWidth}
                              height={barHeight}
                              fill={getTradingAppColor(platform)}
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        title="Memecoin Trading App Protocol Revenue"
        subtitle="Revenue generated by Memecoin Trading Apps on Solana"
      >
...
export default TradingAppRevenueChart; 