...
import { 
  ...
  formatCurrency,
  getDexColor
} from "@/app/api/protocol-revenue/dex-ecosystem/dexRevenueData";
...
interface DexRevenueChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
...
interface ExtendedDexRevenueDataPoint extends DexRevenueDataPoint {
  date?: Date;
}
...
const processDataForBrush = (data: ExtendedDexRevenueDataPoint[]) => {
...
const DexRevenueChart: React.FC<DexRevenueChartProps> = ({ 
  isModalOpen = false, 
  onModalClose = () => {},
  legendsChanged
}) => {
  // State for chart data
  const [rawData, setRawData] = useState<ExtendedDexRevenueDataPoint[]>([]);
  ...
  // Function to prepare stacked bar data from raw data
  const prepareStackedData = useCallback((data: ExtendedDexRevenueDataPoint[]): StackedBarData[] => {
    // Group data by month
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      console.log(`Fetching DEX revenue data...`);
      const revenueData = await fetchDexRevenueData();
      
      if (revenueData.length === 0) {
        console.error('No data returned from API');
        setError('No data available for DEX revenue.');
        setRawData([]);
        setStackedData([]);
        setFilteredData([]);
  ...
  // Format date for display to match the format in RevenueBySegmentChart
  ...
  const renderChartContent = (height: number, width: number, isModal = false) => {
    ...
    const chartMargin = { top: 10, right: 15, bottom: 30, left: 40 };
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        subtitle="Revenue generated by DEXs on Solana"
      >
...
export default DexRevenueChart; 