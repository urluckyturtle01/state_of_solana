...
import { 
  fetchTelegramBotRevenueData, 
  TelegramBotRevenueDataPoint,
  formatCurrency,
  getTelegramBotColor
} from "@/app/api/protocol-revenue/dex-ecosystem/telegramBotRevenueData";
...
interface TelegramBotRevenueChartProps {
  isModalOpen?: boolean;
  onModalClose?: () => void;
...
interface ExtendedTelegramBotRevenueDataPoint extends TelegramBotRevenueDataPoint {
  date?: Date;
}
...
const processDataForBrush = (data: ExtendedTelegramBotRevenueDataPoint[]) => {
...
const TelegramBotRevenueChart: React.FC<TelegramBotRevenueChartProps> = ({ 
  isModalOpen = false, 
  onModalClose = () => {},
  legendsChanged
}) => {
  // State for chart data
  const [rawData, setRawData] = useState<ExtendedTelegramBotRevenueDataPoint[]>([]);
  ...
  const prepareStackedData = useCallback((data: ExtendedTelegramBotRevenueDataPoint[]): StackedBarData[] => {
    // Group data by month
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      console.log(`Fetching Telegram Bot revenue data...`);
      const revenueData = await fetchTelegramBotRevenueData();
      
      if (revenueData.length === 0) {
        console.error('No data returned from API');
        setError('No data available for Telegram Bot revenue.');
        setRawData([]);
        setStackedData([]);
        setFilteredData([]);
        ...
        if (legendsChanged) {
          const legends = sortedPlatforms.slice(0, 5).map(platform => ({
            label: platform,
            color: getTelegramBotColor(platform),
                        ...
                        .forEach(platform => {
                          ...
                          stackedBars.push(
                            <Bar
                              ...
                              width={barWidth}
                              height={barHeight}
                              fill={getTelegramBotColor(platform)}
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <Modal 
        ...
        title="Telegram Bot Protocol Revenue"
        subtitle="Revenue generated by Telegram Bots on Solana"
      >
          ...
          <div className="flex h-full">
            ...
            <div className="w-[10%] h-full pl-3 flex flex-col justify-start items-start">
              ...
              {loading ? (
                // Show loading state
                <>
                  <LegendItem label="Loading..." color="#38bdf8" isLoading={true} />
                  <LegendItem label="Loading..." color="#a855f7" isLoading={true} />
...
export default TelegramBotRevenueChart; 