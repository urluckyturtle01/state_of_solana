...
const SimpleAreaChart: React.FC<SimpleAreaChartProps> = ({ 
  ...
  const { chartData, areaColor, gradientColors } = useMemo(() => {
    ...
    // Skip expensive logging in production
    if (process.env.NODE_ENV === 'development') {
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>, isModal = false) => {
    ...
    const containerRef = isModal ? modalChartRef : chartRef;
    const rect = containerRef.current?.getBoundingClientRect();
    if (!rect || !chartData || chartData.length === 0) return;
    ...
    // Only handle touch if it's within the chart area - be more restrictive
    if (adjustedMouseX >= 0 && adjustedMouseX <= innerWidth && 
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent<HTMLDivElement>, isModal = false) => {
    ...
    // Use requestAnimationFrame to avoid blocking scrolling
    if (tooltip.visible && chartData && chartData.length > 0) {
      requestAnimationFrame(() => {
      handleInteraction(e, isModal);
      });
    }
    // Otherwise, allow normal page scrolling
  }, [handleInteraction, tooltip.visible, chartData]);
  ...
  }, [chartData, xKey, yKey, yFields, areaColor, gradientColors, formatTickValue, error, refreshData, tooltip, handleMouseMove, handleMouseLeave, handleTouchStart, handleTouchMove, handleTouchEnd, isMultiSeries, filterValues, hiddenSeriesState, currentDisplayMode]);
