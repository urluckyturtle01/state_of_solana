...
const preloadChartConfigs = async (pageId: string): Promise<ChartConfig[]> => {
  ...
  try {
    ...
    // Verify that charts with POST API config have their apiEndpoint
    const hasPostApiCharts = charts.some(chart => chart.postApiConfig?.enabled);
    if (hasPostApiCharts) {
      const missingEndpoint = charts.find(chart => chart.postApiConfig?.enabled && !chart.apiEndpoint);
      if (missingEndpoint) {
        console.error(`⚠️ POST API chart ${missingEndpoint.id} is missing apiEndpoint - config may be sanitized`);
      } else {
        console.log(`✅ All POST API charts have apiEndpoint configured`);
      }
    }
...
const batchFetchChartData = async (charts: ChartConfig[], filterValues: Record<string, Record<string, string>>, enableCaching: boolean, urlParams?: URLSearchParams) => {
  ...
  const chartPromises = charts.map(chart => {
    ...
    const cacheKey = `${chart.id}-${chart.apiEndpoint}-${JSON.stringify(chartFilters)}${urlParams ? `-${urlParams.toString()}` : ''}`;
    ...
    return fetchChartData(chart, chartFilters, enableCaching, urlParams)
      .then(data => ({
        chartId: chart.id,
        data,
        fromCache: false
...
const fetchChartData = async (
  ...
  // For POST API charts, skip temp file logic and always fetch from API
  if (chart.postApiConfig?.enabled) {
    console.log(`POST API chart detected: ${chart.id}, fetching live data`);
    const memoryCacheKey = `${chart.id}-${chart.apiEndpoint}-${JSON.stringify(chartFilters)}-${urlParams?.toString()}`;
    return fetchFromApi(chart, chartFilters, memoryCacheKey, cacheEnabled, urlParams);
  }
...
// Helper function to transform parameter values based on transform type
const transformParameterValue = (value: string, transform?: string): any => {
  if (!transform) return value;
  
  switch (transform) {
    case 'date_yyyy_mm_dd':
      // Transform from dd/MM/yy to yyyy-MM-dd
      try {
...
// Helper function to set nested property in object using dot notation
...
const fetchFromApi = async (
  ...
  if (!chart.apiEndpoint && chart.page) {
    try {
      const pageId = chart.page;
      const chartId = chart.id;
      
      console.log(`🚀 DEX TEMP CHART: Fetching chart ${chartId} from page: ${pageId}`);
      const singleChartUrl = `/api/temp-chart-data/${pageId}/${chartId}`;
      
      const response = await fetch(singleChartUrl);
      
      if (response.ok) {
        const result = await response.json();
        const fetchTime = performance.now() - startTime;
        console.log(`✅ Loaded DEX temp chart ${chartId} in ${fetchTime.toFixed(2)}ms (${result.data?.length || 0} rows)`);
        ...
        console.error(`❌ DEX temp chart endpoint failed for ${chartId}: ${response.status}`);
        return [];
      }
    } catch (error) {
      console.error(`❌ Error fetching DEX temp chart ${chart.id}:`, error);
      return [];
    }
  }
  
  // For temp file charts, try individual chart endpoint first (faster!)
  if (chart.apiEndpoint && chart.apiEndpoint.includes('/temp/chart-data/')) {
    try {
      const pageId = chart.page;
      const chartId = chart.id;
      
      console.log(`🚀 Fetching individual chart: ${chartId} from page: ${pageId}`);
      const singleChartUrl = `/api/temp-chart-data/${pageId}/${chartId}`;
      ...
      if (response.ok) {
        const result = await response.json();
        const fetchTime = performance.now() - startTime;
        console.log(`✅ Loaded chart ${chartId} in ${fetchTime.toFixed(2)}ms`);
        ...
        console.log(`⚠️  Individual chart endpoint failed, falling back to full page data`);
      }
    } catch (error) {
      console.log(`⚠️  Error fetching individual chart, falling back:`, error);
    }
  }
  
  // If no apiEndpoint at all, return empty data
  if (!chart.apiEndpoint) {
    console.error(`❌ Chart ${chart.id} has no apiEndpoint and no page property`);
    return [];
  }
  ...
  try {
    ...
    // Check if this is a POST API chart with custom parameter mappings
    if (chart.postApiConfig?.enabled && chart.postApiConfig.parameterMappings) {
      console.log(`Building POST API parameters for chart ${chart.id} from URL params`);
      
      // Map URL parameters to API parameters using the configuration
      chart.postApiConfig.parameterMappings.forEach(mapping => {
        // Try to get value from URL using urlParam first, then apiParamPath, then default
        let value = urlParams?.get(mapping.urlParam);
        
        // If not found with urlParam, try using apiParamPath (for cases like "Block Date.start")
        if (!value && mapping.apiParamPath) {
          value = urlParams?.get(mapping.apiParamPath);
        }
        
        // Fall back to default value if still not found
        if (!value) {
        ...
        if (value) {
          ...
          console.log(`Mapped ${mapping.apiParamPath}=${transformedValue} (from URL or default: ${value})`);
        }
      });
    } else {
      // Regular filter-based parameters
    ...
    if (hasParameters) {
      ...
      console.log(`POST request body for chart ${chart.id}:`, JSON.stringify(requestBody, null, 2));
    }
...
async function getChartConfigsFromTempFile(pageId: string): Promise<ChartConfig[]> {
  try {
    ...
    // Fetch the chart config from temp API route with admin auth to get unsanitized configs
    const response = await fetch(`/api/temp-configs/${pageId}`, {
      headers: {
        'x-admin-auth': 'dashboard-renderer',
    ...
    console.log(`✅ Chart configs loaded with full data (apiEndpoint present: ${pageConfig.charts?.[0]?.apiEndpoint ? 'YES' : 'NO'})`);
...
export default function DashboardRenderer({ 
  ...
  // TRUE PROGRESSIVE LOADING: Load config + data for one chart at a time
  useEffect(() => {
    const startTime = performance.now();
    console.log(`⏱️ [0ms] DashboardRenderer useEffect started for page: ${pageId}`);
    
    setIsClient(true); // Keep for backward compatibility with other checks
    let mounted = true;

    async function loadChartsProgressively() {
      try {
        setIsPageLoading(false); // ✅ SHOW PAGE INSTANTLY (even before configs!)
        console.log(`⏱️ [${(performance.now() - startTime).toFixed(0)}ms] Page loading state set to false`);
        
        console.log(`🚀 ULTRA-FAST LOADING: Loading charts one by one (config → show box → load data → repeat)`);
        
        const configStartTime = performance.now();
        // Load all configs first (but don't wait to display)
        const loadedCharts = await initializeCharts();
        console.log(`⏱️ [${(performance.now() - startTime).toFixed(0)}ms] Configs loaded (took ${(performance.now() - configStartTime).toFixed(0)}ms) - ${loadedCharts.length} charts`);
        
        if (!mounted) return;
        
        let displayedCharts: ChartConfig[] = [];
        
        // Process each chart one by one
        console.log(`⏱️ [${(performance.now() - startTime).toFixed(0)}ms] Starting to render ${loadedCharts.length} charts progressively`);
        ...
        for (let i = 0; i < loadedCharts.length; i++) {
          ...
          // Auto-detect dual-axis BEFORE first render to avoid a second re-render
          ...
          // Small delay for smooth visual progression
          await new Promise(resolve => setTimeout(resolve, 10));
        }
        
        console.log(`⏱️ [${(performance.now() - startTime).toFixed(0)}ms] 🎉 ALL CHARTS LOADED!`);
        
        if (mounted) {
          setAllChartsLoaded(true);
          console.log(`✅ All ${loadedCharts.length} charts loaded progressively!`);
        }
  ...
  // Effect to refetch POST API charts when URL params change
  useEffect(() => {
    if (!urlParams || charts.length === 0 || !isClient) return;

    console.log('🔄 URL params changed, refetching POST API charts...');
    
    // Find POST API charts
    const postApiCharts = charts.filter(chart => chart.postApiConfig?.enabled);
    
    if (postApiCharts.length === 0) {
      console.log('No POST API charts to refetch');
      return;
    }

    console.log(`Found ${postApiCharts.length} POST API charts to refetch`);
    
    // Set loading states for POST API charts
    const loadingStates: Record<string, Partial<{ expanded: boolean; downloading: boolean; screenshotting: boolean; loading: boolean }>> = {};
    postApiCharts.forEach(chart => {
      loadingStates[chart.id] = { loading: true };
    });
    batchUpdateChartStates(loadingStates);

    // Refetch data for POST API charts
    wrappedBatchFetchChartData(postApiCharts, filterValues)
      .then(results => {
        console.log(`✅ Refetched ${results.length} POST API charts`);
        
        const newChartData: Record<string, any[]> = { ...chartData };
        const updatedStates: Record<string, Partial<{ expanded: boolean; downloading: boolean; screenshotting: boolean; loading: boolean }>> = {};
        
        results.forEach(result => {
          newChartData[result.chartId] = result.data || [];
          updatedStates[result.chartId] = { loading: false };
        });
        
        setChartData(newChartData);
        batchUpdateChartStates(updatedStates);
      })
      .catch(error => {
        console.error('Error refetching POST API charts:', error);
        
        // Clear loading states
        const errorStates: Record<string, Partial<{ expanded: boolean; downloading: boolean; screenshotting: boolean; loading: boolean }>> = {};
        postApiCharts.forEach(chart => {
          errorStates[chart.id] = { loading: false };
        });
        batchUpdateChartStates(errorStates);
      });
  }, [urlParams?.toString(), isClient, charts, wrappedBatchFetchChartData, batchUpdateChartStates, filterValues, chartData]);
  ...
  const handleFilterChange = async (chartId: string, filterType: string, value: string) => {
    ...
    // Update filter values using requestAnimationFrame to avoid blocking
  ...
  const updateLegends = useCallback((chartId: string, data: any[]) => {
    ...
    if (isDualAxis && chart.dualAxisConfig) {
      ...
      // Use getValueOrderedColorMap for base-field logic (3 colors for avg/median/max across SOL/USD)
      const getFieldTotal = (f: string) => fieldTotals[f] || 0;
      colorMap = { ...colorMap, ...getValueOrderedColorMap(allFields, getFieldTotal, colorMap) };
      
      // Create legend items for all fields
      chartLegends = allFields
        .filter(field => Math.abs(fieldTotals[field] || 0) > 0.001)
        .map(field => {
    ...
    // Handle area and stacked-area charts
      ...
      if (isDateBased) {
        ...
        if (Array.isArray(chart.dataMapping.yAxis) && chart.dataMapping.yAxis.length > 1) {
          const areaFields = yAxisFields.filter(field => {
          ...
          colorMap = { ...colorMap, ...getValueOrderedColorMap(areaFields, getFieldTotal, colorMap) };
          chartLegends = areaFields.map(field => {
            const total = getFieldTotal(field);
            newLabels.push(field);
            return {
              id: field,
              label: formatFieldName(field),
              color: colorMap[field] || getColorByIndex(areaFields.indexOf(field)),
              value: total,
              shape: 'square' as const
            };
          });
          ...
          if (Math.abs(total) > 0.001) {
            ...
            chartLegends = [{
              id: yFieldName,
              label: formatFieldName(yFieldName),
    ...
    // Then handle regular bar/line charts
      ...
      if (isDateBased) {
        ...
        // Check if we have multiple y-axis fields (multi-series)
        if (Array.isArray(chart.dataMapping.yAxis) && chart.dataMapping.yAxis.length > 1) {
          const dateBasedFields = yAxisFields.filter(field => {
            const total = data.reduce((sum, item) => sum + (Number(item[field]) || 0), 0);
            return Math.abs(total) > 0.001;
          });
          const getFieldTotal = (f: string) => data.reduce((sum, item) => sum + (Number(item[f]) || 0), 0);
          colorMap = { ...colorMap, ...getValueOrderedColorMap(dateBasedFields, getFieldTotal, colorMap) };
          chartLegends = dateBasedFields.map(field => {
            const total = getFieldTotal(field);
            newLabels.push(field);
            return {
              id: field,
              label: formatFieldName(field),
              color: colorMap[field] || getColorByIndex(dateBasedFields.indexOf(field)),
              value: total,
        ...
        if (isMultiSeries) {
          const multiSeriesFields = yAxisFields.filter(field => {
            const total = data.reduce((sum, item) => sum + (Number(item[field]) || 0), 0);
            return Math.abs(total) > 0.001;
          });
          const getFieldTotal = (f: string) => data.reduce((sum, item) => sum + (Number(item[f]) || 0), 0);
          colorMap = { ...colorMap, ...getValueOrderedColorMap(multiSeriesFields, getFieldTotal, colorMap) };
          chartLegends = multiSeriesFields.map(field => {
            const total = getFieldTotal(field);
            newLabels.push(field);
            ...
            return {
              id: field,
              label: formatFieldName(field),
              color: colorMap[field] || getColorByIndex(multiSeriesFields.indexOf(field)),
              value: total,
              shape: isLine ? 'circle' as const : 'square' as const
            };
          });
    ...
    if (isNewColorMap && chartLegends.length > 0) {
      ...
      chartLegends.forEach((legend, index) => {
        const colorKey = legend.id || legend.label;
        newColorMap[colorKey] = colorMap[colorKey] || getColorByIndex(index);
      });
