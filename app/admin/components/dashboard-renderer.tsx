...
export default function DashboardRenderer({ 
  ...
  // Fetch complete data for all charts on initial load
  useEffect(() => {
    if (charts.length === 0) return;
    
    async function fetchAllCompleteData() {
      // Set loading state for all charts at the beginning
      const initialLoadingState: Record<string, boolean> = {};
      ...
      const newCompleteDatasets: Record<string, any[]> = {};
      const newUsingClientFiltering: Record<string, boolean> = {};
      
      // Process charts in batches to avoid overwhelming the API
      const batchSize = 2;
      for (let i = 0; i < charts.length; i += batchSize) {
        const batch = charts.slice(i, i + batchSize);
        
        // Process each chart in the batch concurrently
        await Promise.all(batch.map(async (chart) => {
          try {
            // Create special ALL filter to get complete dataset
            ...
            const response = await fetch(url.toString(), {
              ...
              headers: {
                'Content-Type': 'application/json',
                // Remove x-priority header to avoid CORS issues
              },
              cache: 'force-cache'
            });
            ...
            if (result?.query_result?.data?.rows) {
              completeData = result.query_result.data.rows;
            } else if (Array.isArray(result)) {
              completeData = result;
            } else if (result?.data && Array.isArray(result.data)) {
              completeData = result.data;
            } else if (result?.rows && Array.isArray(result.rows)) {
              completeData = result.rows;
            } else if (result?.results && Array.isArray(result.results)) {
              completeData = result.results;
            }
            
            // Store complete dataset if we got at least some data
            if (completeData.length > 0) {
              newCompleteDatasets[chart.id] = completeData;
              newUsingClientFiltering[chart.id] = true;
              
              // Also update chartData state with the complete data
              setChartData(prev => ({
                ...prev,
                [chart.id]: completeData
              }));
              ...
              newUsingClientFiltering[chart.id] = false;
              
              // Still clear loading state even with no data
              setLoadingCharts(prev => ({
                ...prev,
                [chart.id]: false
              }));
            ...
            // Clear loading state on error and show fallback data
            setLoadingCharts(prev => ({
              ...prev,
              [chart.id]: false
            }));
            ...
            try {
              const fallbackData = await fetchChartDataWithFilters(chart, true);
            } catch (fallbackError) {
      ...
      // Update state with all complete datasets
      setCompleteDatasets(newCompleteDatasets);
      setUsingClientFiltering(newUsingClientFiltering);
      
      // Ensure all charts are marked as loaded in case any were missed
      const finalLoadingState: Record<string, boolean> = {};
      charts.forEach(chart => {
        finalLoadingState[chart.id] = false;
      });
      setLoadingCharts(finalLoadingState);
    }
