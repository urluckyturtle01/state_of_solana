...
export default function ChartCreatorPage() {
  ...
  // Add state for POST API configuration
  const [enablePostApi, setEnablePostApi] = useState(false);
  const [postApiParameters, setPostApiParameters] = useState<Array<{
    urlParam: string;
    apiParamPath: string;
    transform: 'string' | 'number' | 'date_yyyy_mm_dd' | 'date_mm_dd_yy';
    defaultValue: string;
  }>>([]);
        ...
        const loadChartData = async () => {
          try {
            ...
            if (chartToEdit) {
              ...
              // Check for POST API configuration
              if (chartToEdit.postApiConfig?.enabled && chartToEdit.postApiConfig.parameterMappings) {
                setEnablePostApi(true);
                // Map the parameter mappings and provide default values for optional fields
                setPostApiParameters(chartToEdit.postApiConfig.parameterMappings.map(param => ({
                  urlParam: param.urlParam,
                  apiParamPath: param.apiParamPath,
                  transform: param.transform || 'string',
  ...
  const handleValidateApi = async () => {
    ...
    try {
      // Create parameters object from enabled filters or POST API parameters
      const testParameters: Record<string, any> = {};
      let hasMultipleFilters = false;
      let hasParameters = false;
      
      // If POST API mode is enabled, build parameters from postApiParameters
      if (enablePostApi && postApiParameters.length > 0) {
        // Build nested parameters object from POST API configuration
        postApiParameters.forEach(param => {
          if (param.defaultValue) {
            // Use dot notation to set nested properties
            const keys = param.apiParamPath.split('.');
            let current = testParameters;
            ...
            // Set the final value (apply basic transformation for testing)
            const value = param.transform === 'number' ? Number(param.defaultValue) : param.defaultValue;
            current[keys[keys.length - 1]] = value;
          }
        });
        hasParameters = Object.keys(testParameters).length > 0;
      } else {
      ...
      const enabledFilterCount = Object.values(enableFilters).filter(Boolean).length;
        hasMultipleFilters = enabledFilterCount > 1;
        hasParameters = Object.keys(testParameters).length > 0;
      }
      
      // Validate the API endpoint with parameters
      // For POST API mode, the function will automatically use POST if testParameters is provided
    ...
    try {
      ...
      if (enablePostApi && postApiParameters.length > 0) {
        chartConfig = {
          ...
          postApiConfig: {
            enabled: true,
            parameterMappings: postApiParameters
          }
        };
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-2 border-t border-gray-700 pt-6 mt-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-indigo-400">POST API Configuration</h2>
              <FormCheckbox
                id="enablePostApi"
                label="Enable POST API Mode"
                checked={enablePostApi}
                onChange={(e) => setEnablePostApi(e.target.checked)}
                helpText="Enable this for APIs that require POST requests with URL-based parameters"
              />
            </div>
            ...
            {enablePostApi && (
              <div className="space-y-4 bg-gray-900/30 p-4 rounded-lg border border-gray-700">
                <p className="text-sm text-gray-400 mb-4">
                  Configure how URL parameters map to POST API request body parameters. Each mapping defines how a filter value from the URL gets transformed and placed in the API request.
                </p>
                
                {postApiParameters.map((param, index) => (
                  <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                    <div>
                      ...
                      <input
                        type="text"
                        value={param.urlParam}
                        onChange={(e) => {
                          const newParams = [...postApiParameters];
                          newParams[index].urlParam = e.target.value;
                          setPostApiParameters(newParams);
                        }}
                        placeholder="e.g., Block Date"
                        className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-sm text-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={param.apiParamPath}
                        onChange={(e) => {
                          const newParams = [...postApiParameters];
                          newParams[index].apiParamPath = e.target.value;
                          setPostApiParameters(newParams);
                        }}
                        placeholder="e.g., Block Date.start"
                        className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-sm text-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newParams = [...postApiParameters];
                          newParams[index].transform = e.target.value as any;
                          setPostApiParameters(newParams);
                        }}
                        className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-sm text-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      >
                        ...
                        <option value="date_mm_dd_yy">Date (MM/dd/yy)</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-sm text-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        />
                        <button
                          type="button"
                          onClick={() => {
                            const newParams = postApiParameters.filter((_, i) => i !== index);
                            setPostApiParameters(newParams);
                          }}
                          className="px-3 py-2 bg-red-900/30 hover:bg-red-900/50 text-red-400 rounded-md transition-colors"
                ...
                <button
                  ...
                  className="w-full px-4 py-2 bg-indigo-900/30 hover:bg-indigo-900/50 text-indigo-400 rounded-md transition-colors text-sm font-medium"
                >
                ...
                <div className="mt-4 p-3 bg-blue-900/20 border border-blue-800/50 rounded-lg">
                  ...
                  <ul className="text-xs text-blue-200/80 space-y-1 ml-4 list-disc">
                    <li><strong>URL Param:</strong> "Block Date" or "appId" (name used in URL query string)</li>
                    <li><strong>API Param Path:</strong> "Block Date.start" or "app_id" (nested path in POST body)</li>
                    <li><strong>Transform:</strong> How to convert the value (date formats, string, number)</li>
                    <li><strong>Default Value:</strong> Value to use if URL param is missing</li>
                  </ul>
                </div>
              </div>
            )}
