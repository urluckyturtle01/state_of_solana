...
// POST API parameter mapping configuration
export interface PostApiParameterMapping {
  urlParam: string; // Name of the URL parameter (e.g., 'blockDateFrom')
  apiParamPath: string; // Path in the API POST body (e.g., 'Block Date.start')
  transform?: 'string' | 'number' | 'date_yyyy_mm_dd' | 'date_mm_dd_yy'; // Optional transformation
  defaultValue?: string; // Optional default value if URL param is missing
}

// POST API configuration
export interface PostApiConfig {
  enabled: boolean;
  parameterMappings: PostApiParameterMapping[];
}
