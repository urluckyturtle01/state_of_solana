...
export async function getChartConfigsByPage(pageId: string): Promise<ChartConfig[]> {
  ...
  const dbBackedPages = [
    ...
    'dex-tvl',
    'dex-volume',
    'mev-extracted-value-and-pnl',
    'mev-liquidity-hotspot',
    'mev-summary',
    'overview-network-dynamics',
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/temp-configs/${pageId}`, {
      headers: {
        'Cache-Control': 'no-cache'
      }
    });
    ...
    console.log(`Temp file not found for ${pageId}, falling back to S3`);
  } catch (error) {
    console.warn(`Error loading temp file for ${pageId}:`, error);
  }
  
  // Fallback to S3 API if temp file not found
  try {
