...
// Add page-specific caching for charts
...
/**
 * Get chart configurations for a specific page (ULTRA-OPTIMIZED VERSION)
 */
export async function getChartConfigsByPage(pageId: string): Promise<ChartConfig[]> {
  const startTime = performance.now();
  console.log(`⏱️ [Config Load] Getting charts for page: ${pageId}`);
  
  // Check if we have cached charts for this page first
  const cacheKey = `page_${pageId}`;
  
  // IMMEDIATE CACHE RETURN - return cached data instantly if available
  if (CHART_PAGE_CACHE[cacheKey]) {
    const cachedData = CHART_PAGE_CACHE[cacheKey];
    const now = Date.now();
    
    // Return cached data IMMEDIATELY - even if expired, to get <100ms load times
    if (cachedData.charts && cachedData.charts.length > 0) {
      const cacheTime = performance.now() - startTime;
      const isExpired = now - cachedData.timestamp > cachedData.expiresIn;
      
      console.log(`⚡ [Config Load] Instant cache return for page ${pageId} (${cacheTime.toFixed(2)}ms) - ${cachedData.charts.length} charts ${isExpired ? '(expired, refreshing in background)' : '(fresh)'}`);
      
      // If expired, trigger background refresh but still return cached data immediately
      if (isExpired) {
        setTimeout(() => {
          console.log(`🔄 [Config Load] Background refresh started for page ${pageId}`);
          fetchAndCachePageCharts(pageId).catch(error => {
            console.warn(`Background refresh failed for page ${pageId}:`, error);
          });
        }, 0);
      }
  ...
  // IMMEDIATE LOCALSTORAGE RETURN - check localStorage next for near-instant response
  if (typeof window !== 'undefined') {
    const pageStorageKey = `solana-charts-${pageId}`;
    const storedPageCharts = localStorage.getItem(pageStorageKey);
    
    if (storedPageCharts) {
      try {
        const cachedData = JSON.parse(storedPageCharts);
        // Return localStorage data IMMEDIATELY regardless of expiration
        if (cachedData.charts && Array.isArray(cachedData.charts) && cachedData.charts.length > 0) {
          const localTime = performance.now() - startTime;
          const isExpired = cachedData.expires < Date.now();
          
          console.log(`📦 [Config Load] Instant localStorage return for page ${pageId} (${localTime.toFixed(2)}ms) - ${cachedData.charts.length} charts ${isExpired ? '(expired, refreshing in background)' : '(fresh)'}`);
          
          // Cache in memory for even faster subsequent access
          CHART_PAGE_CACHE[cacheKey] = {
            charts: cachedData.charts,
            timestamp: Date.now(),
            expiresIn: CHART_CACHE_DURATION
          };
          
          // Always trigger background refresh for localStorage data to keep it fresh
          setTimeout(() => {
            console.log(`🔄 [Config Load] Background refresh started for page ${pageId} (localStorage)`);
            fetchAndCachePageCharts(pageId).catch(error => {
              console.warn(`Background refresh failed for page ${pageId}:`, error);
            });
          }, 0);
        ...
        console.warn(`Failed to parse cached charts for page ${pageId}:`, e);
      }
    }
    
    // Fallback: check general localStorage (still instant)
    const generalStorageKey = 'solana-charts';
    const generalStoredCharts = localStorage.getItem(generalStorageKey);
    if (generalStoredCharts) {
      try {
        const allLocalCharts = JSON.parse(generalStoredCharts) as ChartConfig[];
        const pageCharts = allLocalCharts.filter(chart => chart.page === pageId);
        
        if (pageCharts.length > 0) {
          const fallbackTime = performance.now() - startTime;
          console.log(`📦 [Config Load] Instant general localStorage return for page ${pageId} (${fallbackTime.toFixed(2)}ms) - ${pageCharts.length} charts`);
          
          // Cache in memory and specific storage
          CHART_PAGE_CACHE[cacheKey] = {
            charts: pageCharts,
            timestamp: Date.now(),
            expiresIn: CHART_CACHE_DURATION
          ...
          // Save to page-specific localStorage for future instant access
          setTimeout(() => {
            try {
              const cacheData = {
                charts: pageCharts,
                expires: Date.now() + CHART_CACHE_DURATION,
                timestamp: Date.now()
              };
              localStorage.setItem(pageStorageKey, JSON.stringify(cacheData));
              console.log(`💾 [Config Load] Saved page-specific cache for ${pageId}`);
            } catch (storageError) {
              console.warn(`Failed to save page-specific cache:`, storageError);
            }
          }, 0);
        ...
        console.warn(`Failed to parse general localStorage charts:`, e);
      }
    }
  }

  // NO CACHE AVAILABLE - Fetch with aggressive optimization
  try {
    console.log(`🌐 [Config Load] No cache available, fetching for page ${pageId}...`);
    
    const freshCharts = await fetchAndCachePageCharts(pageId);
    const totalTime = performance.now() - startTime;
    
    console.log(`✅ [Config Load] Fresh data loaded for page ${pageId} (${totalTime.toFixed(2)}ms) - ${freshCharts.length} charts`);
    return freshCharts;
    
  } catch (error) {
    const errorTime = performance.now() - startTime;
    console.error(`❌ [Config Load] Error fetching charts for page ${pageId} (${errorTime.toFixed(2)}ms):`, error);
    return [];
  }
}

/**
 * Helper function to fetch and cache charts for a specific page (ULTRA-OPTIMIZED)
 */
async function fetchAndCachePageCharts(pageId: string): Promise<ChartConfig[]> {
  ...
  try {
    ...
    // Try page-specific API endpoint with ULTRA-FAST timeout (1 second)
    let response;
    try {
      console.log(`🚀 [Config Load] Ultra-fast API call for page ${pageId}...`);
      
      // Create abort controller with ultra-aggressive 1-second timeout
      const controller = new AbortController();
      const timeoutId = setTimeout(() => {
        console.warn(`⚡ [Config Load] Ultra-fast timeout (1s) for page ${pageId}, aborting API call`);
        controller.abort();
      }, 1000); // Ultra-aggressive 1-second timeout
      
      response = await fetch(`${baseUrl}/api/charts?page=${pageId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Cache-Control': 'max-age=60', // Prefer cached responses
        },
        signal: controller.signal,
        // Add priority hint for faster processing
      ...
      if (response.ok) {
        const data = await response.json();
        const pageCharts = data.charts || [];
        
        const apiTime = performance.now() - fetchStartTime;
        console.log(`✅ [Config Load] Ultra-fast API success for page ${pageId} (${apiTime.toFixed(2)}ms) - ${pageCharts.length} charts`);
        
        // Cache the fresh data
        await cachePageChartsToStorage(pageId, pageCharts);
        return pageCharts;
      } else {
        console.warn(`⚠️ [Config Load] API response not OK (${response.status}) for page ${pageId}`);
        throw new Error(`API response ${response.status}`);
      }
      
    } catch (apiError) {
      const apiErrorTime = performance.now() - fetchStartTime;
      
      if (apiError instanceof Error && apiError.name === 'AbortError') {
        console.warn(`⏰ [Config Load] Ultra-fast API timeout (${apiErrorTime.toFixed(2)}ms) for page ${pageId}, falling back immediately`);
      } else {
        console.warn(`🔄 [Config Load] API error (${apiErrorTime.toFixed(2)}ms) for page ${pageId}:`, apiError);
      }
      
      // IMMEDIATE FALLBACK - Don't try getAllChartConfigs, it's too slow
      // Instead, try to get from any available cache or return empty
      
      // Check if we have any expired cache we can use
      const cacheKey = `page_${pageId}`;
      if (CHART_PAGE_CACHE[cacheKey] && CHART_PAGE_CACHE[cacheKey].charts.length > 0) {
        console.log(`🔄 [Config Load] Using expired memory cache as ultra-fast fallback for page ${pageId}`);
        return CHART_PAGE_CACHE[cacheKey].charts;
      }
        ...
        try {
          ...
          if (storedPageCharts) {
            ...
            if (cachedData.charts && Array.isArray(cachedData.charts)) {
              console.log(`💾 [Config Load] Using localStorage as ultra-fast fallback for page ${pageId} - ${cachedData.charts.length} charts`);
          ...
          // Try general localStorage as absolute final fallback
          const generalStorageKey = 'solana-charts';
          const generalStoredCharts = localStorage.getItem(generalStorageKey);
          if (generalStoredCharts) {
            const allLocalCharts = JSON.parse(generalStoredCharts) as ChartConfig[];
            ...
            if (pageCharts.length > 0) {
              console.log(`📦 [Config Load] Using general localStorage as absolute fallback for page ${pageId} - ${pageCharts.length} charts`);
          ...
          console.warn(`Storage fallback failed:`, storageError);
        }
      }
      
      // If we reach here, we have no cache and API failed - trigger slow fallback in background
      // but return empty array immediately for ultra-fast load
      console.warn(`⚠️ [Config Load] No cache available and API failed for page ${pageId}, returning empty (will try slow fallback in background)`);
      
      // Trigger slow fallback in background (but don't wait for it)
      setTimeout(async () => {
        try {
          console.log(`🐌 [Config Load] Starting slow background fallback for page ${pageId}...`);
          const allCharts = await getAllChartConfigs();
          const pageCharts = allCharts.filter(chart => chart.page === pageId);
          
          if (pageCharts.length > 0) {
            console.log(`🐌 [Config Load] Slow background fallback succeeded for page ${pageId} - ${pageCharts.length} charts`);
            await cachePageChartsToStorage(pageId, pageCharts);
            
            // Trigger a page refresh or state update if needed
            if (typeof window !== 'undefined' && window.location) {
              console.log(`🔄 [Config Load] Background fallback complete - consider refreshing UI`);
              // Could emit a custom event here for the component to listen to
              window.dispatchEvent(new CustomEvent('chartConfigsUpdated', { 
                detail: { pageId, charts: pageCharts } 
              }));
            }
          }
        } catch (backgroundError) {
          console.error(`🐌 [Config Load] Background fallback failed for page ${pageId}:`, backgroundError);
        }
      }, 100);
    ...
    console.error(`❌ [Config Load] Complete fetch failure for page ${pageId} (${errorTime.toFixed(2)}ms):`, error);
    return [];
  }
}

/**
 * Cache page charts to both memory and localStorage
 */
async function cachePageChartsToStorage(pageId: string, charts: ChartConfig[]): Promise<void> {
  const cacheKey = `page_${pageId}`;
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      console.log(`💾 [Config Load] Cached ${charts.length} charts for page ${pageId} in localStorage`);
    } catch (storageError) {
      console.warn(`Failed to cache charts to localStorage for page ${pageId}:`, storageError);
    }
  }
}
