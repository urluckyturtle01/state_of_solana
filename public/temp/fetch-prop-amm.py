...
class DexDataFetcher:
    ...
    def run(self):
        """Run the fetcher for prop_amm folder only."""
        print("\n" + "="*70, flush=True)
        print("🚀 PROP AMM DATA FETCHER - SMART INCREMENTAL", flush=True)
        print("="*70, flush=True)
        print("\n📋 Features:", flush=True)
        ...
        # Process prop_amm folder only
        try:
            self.process_folder('prop_amm')
        except Exception as e:
            print(f"\n❌ Error processing prop_amm: {e}", flush=True)
            import traceback
            traceback.print_exc()
        
        print("\n" + "="*70, flush=True)
        print("🎉 PROP AMM DATA FETCH COMPLETE!", flush=True)
        print("="*70, flush=True)
