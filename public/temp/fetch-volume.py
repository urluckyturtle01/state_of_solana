...
class DexDataFetcher:
    ...
    def modify_sql_incremental(self, sql, last_date, query_run_config):
        ...
        # For volume queries, skip incremental modification as they have hardcoded dates
        # Just return the SQL as-is since they already filter by date
        return sql, False
    ...
    def run(self):
        """Run the fetcher for volume folder only."""
        print("\n" + "="*70, flush=True)
        print("🚀 VOLUME DATA FETCHER - SMART INCREMENTAL", flush=True)
        print("="*70, flush=True)
        print("\n📋 Features:", flush=True)
        ...
        # Process volume folder only
        try:
            self.process_folder('volume')
        except Exception as e:
            print(f"\n❌ Error processing volume: {e}", flush=True)
            import traceback
            traceback.print_exc()
        
        print("\n" + "="*70, flush=True)
        print("🎉 VOLUME DATA FETCH COMPLETE!", flush=True)
        print("="*70, flush=True)
