...
class DexDataFetcher:
    ...
    def run(self):
        """Run the fetcher for tokens folder only."""
        print("\n" + "="*70, flush=True)
        print("🚀 TOKENS DATA FETCHER - SMART INCREMENTAL", flush=True)
        print("="*70, flush=True)
        print("\n📋 Features:", flush=True)
        ...
        # Process tokens folder only
        try:
            self.process_folder('tokens')
        except Exception as e:
            print(f"\n❌ Error processing tokens: {e}", flush=True)
            import traceback
            traceback.print_exc()
        
        print("\n" + "="*70, flush=True)
        print("🎉 TOKENS DATA FETCH COMPLETE!", flush=True)
        print("="*70, flush=True)
