...
class DexDataFetcher:
    ...
    def run(self):
        """Run the fetcher for network_fees folder only."""
        print("\n" + "="*70, flush=True)
        print("🚀 NETWORK FEES DATA FETCHER - SMART INCREMENTAL", flush=True)
        print("="*70, flush=True)
        print("\n📋 Features:", flush=True)
        ...
        # Process network_fees folder only
        try:
            self.process_folder('network_fees')
        except Exception as e:
            print(f"\n❌ Error processing network_fees: {e}", flush=True)
            import traceback
            traceback.print_exc()
        
        print("\n" + "="*70, flush=True)
        print("🎉 NETWORK FEES DATA FETCH COMPLETE!", flush=True)
        print("="*70, flush=True)
