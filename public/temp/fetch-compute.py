...
class DexDataFetcher:
    ...
    def modify_sql_incremental(self, sql, last_date, query_run_config):
        ...
        # Add WHERE clause for incremental fetch
        # Look for WHERE clause and add date filter
        if 'WHERE' in sql.upper():
            # Add AND condition
            sql = sql.replace('WHERE', f"WHERE date >= DATE '{date_str}' AND", 1)
        else:
            # Add WHERE clause before GROUP BY or ORDER BY
            if 'GROUP BY' in sql.upper():
                sql = sql.replace('GROUP BY', f"WHERE date >= DATE '{date_str}' GROUP BY", 1)
            elif 'ORDER BY' in sql.upper():
                sql = sql.replace('ORDER BY', f"WHERE date >= DATE '{date_str}' ORDER BY", 1)
            else:
    ...
    def merge_data(self, existing_data, new_data, is_cumulative):
        ...
        if is_cumulative:
            ...
            new_dates = set(new_df['date'].values)
    ...
    def save_data(self, page_id, folder, config, data, sql_name):
        ...
        chart_ids_this_sql = {chart['id'] for chart in charts_data}
    ...
    def process_folder(self, folder_name):
        ...
        for sql_file in sql_files:
            ...
            if is_incremental:
                last_date = self.get_last_date(page_id)
                if last_date:
                    ...
                    if was_modified:
                        ...
                        try:
                            ...
                            if new_df is not None:
                                if not new_df.empty:
                                        ...
                                        try:
                                            with open(data_file) as f:
                                                ...
                                                for chart in page_data.get('charts', []):
                                                    if chart.get('id') == primary_chart_id:
                                    ...
                                    merged_data = self.merge_data(existing_data, new_data, is_cumulative)
                                    merged_df = pd.DataFrame(merged_data)
                                    
                                    # Save
                                    self.save_data(page_id, folder_name, config, merged_df, sql_name)
                                else:
                        ...
                        if df is not None and not df.empty:
                            ...
                            self.save_data(page_id, folder_name, config, df, sql_name)
                        elif df is not None:
    ...
    def run(self):
        """Run the fetcher for compute folder only."""
        print("\n" + "="*70, flush=True)
        print("🚀 COMPUTE DATA FETCHER - SMART INCREMENTAL", flush=True)
        print("="*70, flush=True)
        print("\n📋 Features:", flush=True)
        ...
        # Process compute folder (base_path already points to dex-trades/compute, so we use 'compute')
        try:
            self.process_folder('compute')
        except Exception as e:
            print(f"\n❌ Error processing compute: {e}", flush=True)
            import traceback
            traceback.print_exc()
        
        print("\n" + "="*70, flush=True)
        print("🎉 COMPUTE DATA FETCH COMPLETE!", flush=True)
        print("="*70, flush=True)
