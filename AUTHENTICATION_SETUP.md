...
INTERNAL_AUTH_PASSWORD=your-sf-dashboards-password-here
...
- `/explorer` - Blockchain explorer (requires Google OAuth)
- `/dashboards` - Custom dashboards (requires Google OAuth)
- `/sf-dashboards` - SF Foundation dashboards (requires internal password)
- `/dflow` - DFlow dashboards (requires DFlow password)
...
### Google OAuth (Explorer/Dashboards)
...
### Internal Password (SF Dashboards/DFlow)
1. User clicks on internal route (SF Dashboards/DFlow)
2. App checks authentication status via localStorage/cookies
3. If not authenticated, login modal opens
4. User enters the section-specific password
5. Password is verified on the server side
6. After successful authentication, user can access the section
7. Authentication persists for 7 days via localStorage and cookies
...
- Internal password authentication uses environment variables for security
- SF Dashboards use `INTERNAL_AUTH_PASSWORD` environment variable
- DFlow section uses `DFLOW_AUTH_PASSWORD` environment variable (default: dflow123)
- Both internal auth methods share the same authentication storage for simplicity 